...
exports.onWriteCurriculumProfile = functions.firestore.document("curriculumProfile/{curriculumProfileId}")
  .onWrite(async (change, context) => {
    let isDeleted = false;
    let curriculumProfile;
    if (!change.after.exists) {
      isDeleted = true;
      curriculumProfile = change.before.data();
      curriculumProfile.id = change.before.ref.id;
    }
    else {
      curriculumProfile = change.after.data();
      curriculumProfile.id = change.after.ref.id;
    }    
    const fb = helper.getFirebaseApp();
    let auth = fb.auth();
    let user = await auth.getUser(curriculumProfile.uid);
    if (user) {
      if (!isDeleted) {
        let currentCustomClaims = user.customClaims;
        currentCustomClaims['curriculumProfileId'] = context.params.curriculumProfileId;
        return auth.setCustomUserClaims(user.uid, currentCustomClaims);
      }
      else {
        let currentCustomClaims = user.customClaims;
        delete currentCustomClaims['curriculumProfileId'];
        return auth.setCustomUserClaims(user.uid, currentCustomClaims);
      }
    }
    return null;
