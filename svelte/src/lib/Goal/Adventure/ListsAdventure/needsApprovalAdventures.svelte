<script>
  ...
  function compareTimeLeftToNeedsWork(a, b) {
  ...
  $: {
    adventureArray = goal.needsApprovalAdventures;
    adventureArray.sort(compareTimeLeftToNeedsWork);
  }

  $: {
    for (let i = 0; i < adventureArray.length; i++) {
      let adventure = adventureArray[i];
      adventure.timeInFutureToNeedsWork =
        adventure.inNeedsForApprovalAt + daysToSendToNeedsWork;
    }
  }
...
<SecondTabs bind:goal mainSelected="needs-approval" />

<!-- <DataTable bind:goalId={goal.id} bind:adventureArray={goal.needsApprovalAdventures} bind:firebase/> -->
