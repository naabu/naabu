<script>
  ...
  function compareTimeLeftToTrash(a, b) {
  ...
  $: {
    adventureArray = goal.needsWorkAdventures;
    adventureArray.sort(compareTimeLeftToTrash);
  }

  $: {
    for (let i = 0; i < adventureArray.length; i++) {
      let adventure = adventureArray[i];
      adventure.timeInFutureToTrash =
        adventure.inNeedsWorkAt + daysToSendToTrash;
    }
  }
...
<SecondTabs bind:goal mainSelected="needs-work" />

<!-- <DataTable bind:goalId={goal.id} bind:adventureArray={goal.needsWorkAdventures} bind:firebase/> -->
