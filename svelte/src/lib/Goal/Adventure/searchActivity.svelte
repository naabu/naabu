 <script>
  ...
  import { getAlgoliaSearchClient, getActivityIndex } from "$lib/algolia";
  ...
  async function search() {
    ...
    for (let i = 0; i < hits.length; i++) {
      hits[i].descriptionRaw = truncate(
        hits[i].descriptionRaw,
        truncateDescription
      );
    }
  }
  ...
  <div class="relative flex items-center justify-center mt-4 mb-4">
    <div class="w-full sm:max-w-xs">
      ...
      <div class="relative">
        ...
        <input id="search" name="search" class="pr-3 block w-full bg-white border border-gray-300 rounded-md py-2 pl-10 text-sm placeholder-gray-500 focus:outline-none focus:text-gray-900 focus:placeholder-gray-400 focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" placeholder="Search" type="search" bind:value={query} on:keyup={search} on:search={search}>
      </div>
    </div>
  </div>
</div>
              ...
              <tr>
                ...
                <td class="px-6 py-4">
                  <div class="description text-sm text-gray-900">
                    {#if hit.descriptionRaw}{@html hit.descriptionRaw}{:else}Geen
                      beschrijving{/if}
                  </div>
                </td>
                <td
                ...
                >
                  <a
                    href="avontuur-maken/{hit.objectID}"
