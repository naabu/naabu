<script>
  ...
  async function setToNeedsApproval() {
    if (connection.status === "in-progress") {
      ...
      connection.status = "needs-approval";
    ...
    <div class="flex">
      <div class="ml-auto mt-4">
        {#if connection.status === "in-progress"}
          ...
          >
            Verbinding klaar om te publiceren
          </button>
        {/if}
        {#if connection.status === "needs-approval"}
          <button
    ...
    <div class="max-w-lg mx-auto px-6 mt-16">
      <div class="flow-root">
        <ul role="list" class="-mb-8">
          {#each connection.updates as update, i}
            <li>
              <div class="relative pb-8">
                {#if i + 1 !== connection.updates.length}
                  <span
